...
const repurchaseTask = require("../../taskQueue/repurchaseTask");
...
const generateUserProductRepurchaseInvoice = async (repurchaseId) => {
  // Step 2: Check if invoice number already exists

  const invoiceNumber = await generateProductRepurchaseInvoiceNumber();

  const repurchaseData = await prisma.repurchase.update({
  ...
  try {
    ...
    logger.info("Error deleting repurchase invoice or folder:", err);
  }
  // end
