...
const impersonateUser = async (req, res) => {
  try {
    ...
    if (admin?.role !== ADMIN) {
      return res
        .status(403)
        .json({ message: "Only admins can impersonate users." });
    }
    ...
    return res.json({
      ...
      user: {
        ...
        status: targetUser?.member?.status,
      },
    });
  } catch (error) {
    res.status(500).json({
      message: "Failed TO VISIT ACCOUNT",
      details: error.message,
    });
  }
};
...
const backToAdmin = async (req, res) => {
  try {
    const { impersonating, originalAdminId } = req.user;

    if (!impersonating || !originalAdminId) {
      return res.status(403).json({ message: "Not impersonating any user." });
    }
    ...
    if (!admin || admin.role !== ADMIN) {
      return res.status(403).json({ message: "Original admin not found." });
    }

    const token = jwt.sign({ userId: admin.id }, jwtConfig.secret, {
      expiresIn: jwtConfig.expiresIn,
    });
    ...
    res.status(500).json({
      message: "Error In Back To Admin",
      details: error.message,
    });
  }
};
