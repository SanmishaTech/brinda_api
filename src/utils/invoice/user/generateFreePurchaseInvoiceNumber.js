...
async function generateFreePurchaseInvoiceNumber() {
  const now = dayjs();
  const prefix = `F-${now.format("MMYY")}`; // e.g., I-0625

  // Find the latest username that starts with this prefix
  const latest = await prisma.freePurchase.findFirst({
    where: {
      invoiceNumber: {
        startsWith: prefix,
      },
...
module.exports = { generateFreePurchaseInvoiceNumber };
